include "GpioExpander.hh";


GpioExpander::GpioExpander(const int A2, const int A1, const int A0, const std::vector<GpioType> io_pins, const int joystick_start_index) {
    uint8_t pcf_id = 0U;
    pcf_id << A2 << A1 << A0;
...
void GpioExpander::set_led(const uint8_t pin_id, const uint8_t value) {
    gpio_board_.write(pin_id, value);
}

void GpioExpander::set_all_led(const uint8_t value) {
...
void GpioExpander::setup() {
    gpio_board_.begin();
}

void GpioExpander::loop(Joystick_ &joystick){
